...
- **绝不改动你的笔记。** 译文只作为渲染层 DOM 注入；注册的 Markdown post-processor 是
  无网络的渲染边界，DOM 适配器负责兼容 Obsidian 的虚拟化阅读视图。关闭再重开笔记，
  磁盘上的文件一字节未变。
- **绝不自动翻译。** 打开/切换笔记、布局变化、滚动渲染和修改设置都不会发送翻译请求。
  翻译**有且仅有**在你显式点击悬浮按钮 / 状态栏按钮（或执行命令）时才会运行。
...
- **跳过不该翻译的内容：** 代码块、公式、纯图片块、裸 URL、纯符号/数字块，以及能可靠识别为
  特征文字目标语言（`zh`、`ja`、`ko`、`th`、`he`、`el`）的块。英文、越南语等共用拉丁
  字母的语言会保守地继续翻译，因为仅凭字母体系无法可靠区分不同语言。
...
- **远程服务。** 只有在你主动触发翻译或点击**测试连接**时，插件才会把当前笔记中可翻译的
  段落文本发送到当前 LLM 预设所配置的端点（默认 `https://api.deepseek.com`）；传统机器翻译
  则使用各服务固定的官方 API 端点。除此之外不发送任何数据，也绝不会同时发给多个服务。
...
v0.3.0 使用 settings schema v2，会把 v0.2.5 的扁平设置一次性迁移到新格式，并在改写
...
| 目标语言 | `zh-CN` | 一等预设包括 `zh-CN`、`zh-TW`、`en`、`ja`、`ko`、`vi` 等，也可输入自定义的 BCP-47 风格代码。 |
| 默认显示模式 | 双语对照 | |
...
| 自定义指令 _（仅 LLM）_ | _（空）_ | 可选，追加到系统提示词之后——术语表、语气或领域。非空指令会纳入缓存身份，修改后将在下一次显式翻译时生效。 |
...
src/
  ...
  translator/   provider.ts（接口 + 类型化错误 + 共享 HTTP 助手）、每个后端一个
                ...
                langCodes.ts（各服务目标语言码映射）、cache.ts（LRU + 序列化）、
                cachePersistence.ts（只在插件目录加载/写入/删除 cache.json）；
...
纯逻辑模块绝不依赖 `obsidian` 运行时（它是一个仅类型的包）。薄运行时外壳通过 Obsidian stub
和轻量 DOM 测试；provider 的 HTTP 调用被隔在可注入的 `HttpClient` 接缝之后，因此真实的
请求构造/响应解析路径可以脱离网络验证。DOM 收集/注入、controller 的显式触发边界、
`requestUrl` 适配器和缓存持久化也都有针对性测试。

LLM 路径的批次以带编号的 `<<<SEG k>>>` sentinel 打包；每次 `provider.translate()` 严格只发
一个 HTTP 请求。若模型返回的段数不对，该批次会明确失败，并等待用户下一次显式点击翻译重试。
传统机器翻译 API 每请求只译一段文本、天然一一对应，因此完全绕过 sentinel 协议；各 provider
声明自己的单请求硬上限，controller 按上限切分批次。
