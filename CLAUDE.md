...
为 Obsidian 开发一个「沉浸式翻译」风格的插件：在**阅读模式**下，把当前外语文章逐段翻译成中文，
...
插件名**不要直接叫 "Immersive Translate"**（是沉浸式翻译的品牌，有商标风险，也过不了社区插件命名规范）。
项目/插件正式定名：**Interlinear**（id `interlinear`）。README 里注明「灵感来自沉浸式翻译」即可。
